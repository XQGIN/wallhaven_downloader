...
try:
    ...
    from wallhaven_downloader.utils.accessibility_manager import get_accessibility_manager
...
class HoverableComboBox(QComboBox):
    ...
    def _create_arrow_icons(self):
        ...
        self._down_arrow_hover_path = os.path.join(temp_dir, "main_combobox_down_arrow_hover.png")
...
class EnhancedSpinBox(QSpinBox):
    ...
    def _create_arrow_icons(self):
        ...
        self._up_arrow_hover_path = os.path.join(temp_dir, "main_spinbox_up_arrow_hover.png")
        self._down_arrow_path = os.path.join(temp_dir, "main_spinbox_down_arrow.png")
        self._down_arrow_hover_path = os.path.join(temp_dir, "main_spinbox_down_arrow_hover.png")
...
class MainWindow(QMainWindow):
    ...
    def applyTheme(self):
        ...
        if hasattr(self, "modern_ui_manager") and self.modern_ui_manager and self.modern_ui_manager.is_initialized():
            try:
    ...
    def _apply_main_stylesheet(self):
        """根据当前主题刷新主窗口基础样式"""
        try:
            tm = self.theme_manager if hasattr(self, "theme_manager") and self.theme_manager else get_enhanced_theme_manager()
    ...
    def forceQuit(self):
        ...
        if self.download_thread and self.download_thread.isRunning():
            ...
            reply = QMessageBox.question(self, self.i18n.t("message.confirm"), self.i18n.t("message.downloading_confirm"),
    ...
    def initUI(self):
        ...
        if not hasattr(self, 'liquid_glass_manager') or self.liquid_glass_manager is None:
            try:
                from ui.liquid_glass.liquid_glass_manager import LiquidGlassManager
            except ImportError:
                from wallhaven_downloader.ui.liquid_glass.liquid_glass_manager import LiquidGlassManager
            try:
        ...
        if self.liquid_glass_manager:
            left_panel = self.liquid_glass_manager.create_glass_panel(self.content_widget, panel_type="elevated")
        else:
        ...
        download_settings_layout.addRow(self.i18n.t("download.start_page") + ":", self.start_page_spin)
        
        # 下载页数
        self.page_count_spin = EnhancedSpinBox()
        self.page_count_spin.setRange(1, 999999)  # 设置为很大的数，表示不限页数
        self.page_count_spin.setValue(1)
        download_settings_layout.addRow(self.i18n.t("download.page_count") + ":", self.page_count_spin)
        ...
        download_settings_layout.addRow(self.i18n.t("download.dir") + ":", download_dir_layout)
        ...
        if self.liquid_glass_manager:
            right_panel = self.liquid_glass_manager.create_glass_panel(self.content_widget, panel_type="floating")
        else:
    ...
    def onDuplicateDetected(self, duplicate_count, total_count):
        ...
        # 更新状态标签，显示重复文件信息
        self.status_label.setText(self.i18n.t("status.fetching_with_duplicates", count=duplicate_count))
    ...
    def stopDownload(self):
        ...
        if self.download_thread and self.download_thread.isRunning():
            ...
            # 显示停止后的提示信息
            QMessageBox.information(self, self.i18n.t("message.download_stopped"), 
        ...
        if hasattr(self.download_thread, 'duplicate_images') and self.download_thread.duplicate_images > 0:
            ...
            if self.modern_ui_manager and self.modern_ui_manager.is_initialized():
                self.modern_ui_manager.show_success(
                    self.i18n.t("message.download_completed_with_duplicates", count=self.download_thread.duplicate_images),
                    duration=5000
                )
            else:
                QMessageBox.information(self, self.i18n.t("message.download_completed"), 
                                      self.i18n.t("message.download_completed_with_duplicates", count=self.download_thread.duplicate_images))
        else:
            # 使用 Toast 通知（如果可用）
            if self.modern_ui_manager and self.modern_ui_manager.is_initialized():
                self.modern_ui_manager.show_success(self.i18n.t("message.download_completed_msg"), duration=3000)
            else:
                QMessageBox.information(self, self.i18n.t("message.download_completed"), self.i18n.t("message.download_completed_msg"))
    ...
    def downloadFailed(self, error_msg):
        ...
        # 使用 Toast 通知（如果可用）
        if self.modern_ui_manager and self.modern_ui_manager.is_initialized():
            self.modern_ui_manager.show_error(self.i18n.t("message.download_failed_msg", error=error_msg), duration=5000)
        else:
            QMessageBox.critical(self, self.i18n.t("message.download_failed"), self.i18n.t("message.download_failed_msg", error=error_msg))
    ...
    def _apply_liquid_glass_effects(self):
        ...
        try:
            ...
            if self.liquid_glass_manager.initialize():
                ...
                logger.info("液态玻璃效果应用成功（已禁用窗口级阴影，改由 paintEvent 内部光晕呈现）")
    ...
    def _setup_accessibility_features(self):
        ...
        try:
            ...
            # 为主窗口设置辅助功能标签
            self.accessibility_manager.set_aria_label(self, self.i18n.t("app.name") + " - " + self.i18n.t("window.settings"))
    ...
    def paintEvent(self, event):
        ...
        if not self.isMaximized():
            ...
            # 绘制圆角矩形边框（光晕效果）
            painter.drawRoundedRect(QRectF(rect.adjusted(1, 1, -1, -1)), border_radius, border_radius)
    ...
    def resizeEvent(self, event):
        ...
        if self.isMaximized():
            ...
            # 创建圆角区域
            region = QRegion(QRect(0, 0, self.width(), self.height()), QRegion.Rectangle)
