...
class WallpaperDownloadThread(QThread):
    ...
    def __init__(
        ...
        # 优化: 增加默认并发数到5，提高下载速度
        self.concurrent_downloads = max(concurrent_downloads, 5)
        self.max_retries = 3
        ...
        # 预览图生成频率配置
        self.preview_generation_interval = 5  # 每 N 张生成一次预览
    ...
    def _create_session(self) -> requests.Session:
        ...
        adapter = HTTPAdapter(
            ...
            pool_connections=20,  # 增加到20
            pool_maxsize=50       # 增加到50
        )
        ...
        try:
            ...
            for retry_count in range(self.max_retries):
                try:
                    ...
                    wait_time = min(2 ** retry_count, 5)
                    logger.debug(f"网络错误: {type(e).__name__}, 重试 {retry_count + 1}/{self.max_retries}")
                    time.sleep(wait_time)
    ...
    def run(self):
        ...
        try:
            ...
            logger.info(f"发现 {len(existing_files)} 个已存在文件")
