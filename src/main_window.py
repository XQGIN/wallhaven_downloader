...
class GlassEffectWidget(QWidget):
    ...
    def _updateBackgroundCache(self):
        ...
        for i in range(self._shadow_blur // 2):
            alpha = int(
                self._shadow_color.alpha() * (1 - i / (self._shadow_blur // 2)) * 0.4
            )
            color = QColor(
                self._shadow_color.red(),
                self._shadow_color.green(),
...
class ImagePreviewWidget(QWidget):
    ...
    def addImage(self, file_path, pixmap):
        ...
        # 存储图片信息
        self.all_images.append({"file_path": file_path, "pixmap": fixed_pixmap})
    ...
    def updateDisplay(self):
        ...
        # 计算总页数
        total_pages = max(
            1, (len(self.all_images) + self.ITEMS_PER_PAGE - 1) // self.ITEMS_PER_PAGE
        )
...
class SettingsDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.transparency_slider.valueChanged.connect(
            lambda v: self.transparency_label.setText(str(v))
        )
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        # 排序设置
        self.sorting_group = QGroupBox("排序方式")
        sorting_layout = QFormLayout()

        # 排序方式选择
        self.sorting_combo = HoverableComboBox()
        self.sorting_combo.addItem("相关度", "relevance")
        self.sorting_combo.addItem("随机", "random")
        self.sorting_combo.addItem("最新添加", "date_added")
        self.sorting_combo.addItem("浏览量", "views")
        self.sorting_combo.addItem("收藏数", "favorites")
        self.sorting_combo.addItem("排行榜", "toplist")
        self.sorting_combo.addItem("热门", "hot")
        sorting_layout.addRow("排序:", self.sorting_combo)
    ...
    def onDuplicateDetected(self, duplicate_count, total_count):
        ...
        self.status_label.setText(
            f"正在获取图片列表... (检测到 {duplicate_count} 个重复文件)"
        )
